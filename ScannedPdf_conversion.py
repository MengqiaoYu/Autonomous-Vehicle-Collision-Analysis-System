...
def find_loc(img_thirdpage, checkmark):
    ...
    npimg_thirdpage_col1 = npimg_thirdpage[300:2600,1575:1925]
    # import pdb; pdb.set_trace()
    plt.imshow(npimg_thirdpage_col1)
    col_1 = scipy.signal.convolve(
        np.int32(npimg_thirdpage_col1.mean(-1)),
        255-checkmark.mean(-1).astype(np.uint32),mode='valid')
    col_1 =col_1 / col_1.max() * 255

    if DEBUG:
        fig_result = plt.figure()
        plt.imshow(np.uint8(np.logical_and(1, col_1<160)), cmap='gray')
        plt.show()
        plt.savefig('result.jpg')
        import pdb; pdb.set_trace()

    cm_pixel_row = np.where(col_1 < 160)[0] // 75
    cm_pixel_col = np.where(col_1 < 160)[1] // 126
    ...
    # loc_2 = np.unique(np.where(col_1 < 160)[1]//126, return_counts=True)
